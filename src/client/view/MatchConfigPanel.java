...
public class MatchConfigPanel extends BaseDecoratedPanel {
	...
	public MatchConfigPanel() {
		...
		gbc.insets = UiScaleConfig.scaleInsets(10, 20, 10, 20);
		gbc.fill = GridBagConstraints.HORIZONTAL;

		// タイトルラベル
		titleLabel = new JLabel("対戦設定", SwingConstants.CENTER);
		titleLabel.setFont(UiScaleConfig.scaleFont("Meiryo", Font.BOLD, 26));
		titleLabel.setForeground(Color.WHITE);
		gbc.gridx = 0;
		gbc.gridy = 0;
		gbc.gridwidth = 2;
		gbc.insets = UiScaleConfig.scaleInsets(20, 20, 20, 20);
		dialogPanel.add(titleLabel, gbc);

		// ユーザー名ラベル
		JLabel userNameLabel = new JLabel("ユーザー名");
		userNameLabel.setFont(UiScaleConfig.scaleFont("Meiryo", Font.PLAIN, 16));
		userNameLabel.setForeground(Color.WHITE);
		gbc.gridx = 0;
		gbc.gridy = 1;
		gbc.gridwidth = 1;
		gbc.insets = UiScaleConfig.scaleInsets(10, 20, 5, 10);
		gbc.anchor = GridBagConstraints.WEST;
		dialogPanel.add(userNameLabel, gbc);

		// ユーザー名入力フィールド
		userNameField = new JTextField(15);
		userNameField.setFont(UiScaleConfig.scaleFont("Meiryo", Font.PLAIN, 14));
		userNameField.setPreferredSize(UiScaleConfig.scale(200, 30));
		gbc.gridx = 1;
		gbc.gridy = 1;
		gbc.insets = UiScaleConfig.scaleInsets(10, 10, 5, 20);
		dialogPanel.add(userNameField, gbc);

		// 部屋番号ラベル
		roomNumberLabel = new JLabel("ルーム番号");
		roomNumberLabel.setFont(UiScaleConfig.scaleFont("Meiryo", Font.PLAIN, 16));
		...
		roomNumberField.setFont(UiScaleConfig.scaleFont("Meiryo", Font.PLAIN, 14));
		roomNumberField.setPreferredSize(UiScaleConfig.scale(200, 30));
		gbc.gridx = 1;
		gbc.gridy = 2;
		gbc.insets = UiScaleConfig.scaleInsets(10, 10, 5, 20);
		...
		// ボタンパネル
		JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, UiScaleConfig.scale(20), 0));
	...
	private JPanel createDialogPanel() {
		JPanel panel = new BaseDecoratedPanel() {
			@Override
			protected void paintPanel(Graphics2D g2d) {
				int arc = UiScaleConfig.scale(20);
				int stroke = UiScaleConfig.scale(2);
				// 角丸の背景
		...
		panel.setPreferredSize(UiScaleConfig.scale(460, 320));
		return panel;
	}
	...
	private JButton createStyledButton(final String text, final Color color) {
		...
		button.setFont(UiScaleConfig.scaleFont("Meiryo", Font.BOLD, 15));
		button.setForeground(Color.WHITE);
		button.setBackground(color);
		button.setPreferredSize(UiScaleConfig.scale(160, 40));
		button.setFocusPainted(false);
		Insets padding = UiScaleConfig.scaleInsets(5, 15, 5, 15);
